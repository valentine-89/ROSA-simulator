(function () {
  ...
  var language = "vi";
  var phase = "";
  var setupTitle = "Cài đặt tưới cây tuần tự";
  ...
  function setupTitleText() {
    ...
    if (true) return "Cài đặt tưới cây tuần tự";
  ...
  function listTitle() {
    ...
    if (true) return "Danh sách tuyến tưới";
    return "";
  }
  ...
  function addLabel() {
    ...
    if (true) return "Thêm tuyến tưới";
    return text.add;
  }
  ...
  function setupShell() {
    ...
    var withList = false || false || false || true;
  ...
  function makeSequentialDevice(index, source) {
    if (typeof index !== "number") {
      var nextSource = index;
      index = typeof source === "number" ? source : 0;
      source = nextSource;
    }
  ...
  function render() {
    ...
    els.countBadge.textContent = state.items.length + " " + (false ? text.cabinet : true ? "tuyến" : text.group);
    notifyResize();
  }
  ...
  function renderItem(item, index) {
    ...
    if (true) return renderSequentialDevice(item, index);
  ...
  function blankItem() {
    ...
    if (true) return { title: "Tuyến tưới " + (state.items.length + 1), sessionId: state.context.sessionId || "", valveCount: 12 };
  ...
  function cloneBasic(source) {
    ...
    if (true) return { title: safeText(source.title, "Tuyến tưới") + " copy", sessionId: source.sessionId || state.context.sessionId || "", valveCount: source.valveCount || source.valves && source.valves.count || 12, pump: clone(source.pump || {}), valves: clone(source.valves || {}) };
