(function () {
  ...
  async function readTelemetry(ioid) { try { var data = await jsonRequest(telemetryUrl(ioid)); return data && data.c2 ? { serverTime: Number(data.c2.serverTime || 0), payload: data.c2.payload || {} } : null; } catch (_) { return null; } }
  ...
  async function auditSetting(ioid, key, value, resultState) { try { await runMacro(cfg.fleetAdminPageId, "biomass-setting-audit", { ioid: ioid, setting_key: key, value: value, state: resultState }); } catch (_) {} }
  ...
  els.addForm.onsubmit = async function (event) { event.preventDefault(); var form = new FormData(els.addForm); var params = { ioid: String(form.get("ioid") || "").trim(), name: String(form.get("name") || "").trim(), location: String(form.get("location") || "").trim() }; if (String(form.get("latitude") || "").trim() && String(form.get("longitude") || "").trim()) { params.latitude = Number(form.get("latitude")); params.longitude = Number(form.get("longitude")); } try { await runMacro(cfg.fleetAdminPageId, "biomass-fleet-create", params); showModal(els.addModal, false); notify("Đã thêm lò"); refresh(); } catch (error) { notify(error.message, true); } };
  ...
  els.deviceForm.onsubmit = async function (event) { event.preventDefault(); if (!state.current) return; var form = new FormData(els.deviceForm); try { await runMacro(cfg.fleetAdminPageId, "biomass-fleet-update", { ioid: state.current.ioid, name: String(form.get("name") || "").trim(), location: String(form.get("location") || "").trim(), latitude: Number(form.get("latitude")), longitude: Number(form.get("longitude")) }); notify("Đã lưu"); await openSettings(state.current.ioid); refresh(); } catch (error) { notify(error.message, true); } };
  ...
  els.deleteBurner.onclick = async function () { if (!state.current || !confirm("Xóa " + state.current.ioid + "?")) return; try { await runMacro(cfg.fleetAdminPageId, "biomass-fleet-delete", { ioid: state.current.ioid }); showModal(els.settingsModal, false); notify("Đã xóa lò"); refresh(); } catch (error) { notify(error.message, true); } };
