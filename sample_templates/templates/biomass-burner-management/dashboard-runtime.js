(function () {
  ...
  var els = { title: $("bb-title"), subtitle: $("bb-subtitle"), status: $("bb-status"), refresh: $("bb-refresh"), addOpen: $("bb-add-open"), addModal: $("bb-add-modal"), addForm: $("bb-add-form"), addClose: $("bb-add-close"), addCancel: $("bb-add-cancel"), search: $("bb-search"), pageSize: $("bb-page-size"), apply: $("bb-apply-filter"), body: $("bb-table-body"), empty: $("bb-empty"), prev: $("bb-prev"), next: $("bb-next"), pageInfo: $("bb-page-info"), total: $("bb-kpi-total"), active: $("bb-kpi-active"), stale: $("bb-kpi-stale"), burned: $("bb-kpi-burned"), map: $("bb-map"), mapLoading: $("bb-map-loading"), mapFit: $("bb-map-fit"), mapList: $("bb-map-list"), mapCount: $("bb-map-count"), settingsModal: $("bb-settings-modal"), settingsTitle: $("bb-settings-title"), settingsSubtitle: $("bb-settings-subtitle"), settingsClose: $("bb-settings-close"), deviceForm: $("bb-device-form"), settingsGrid: $("bb-settings-grid"), settingsState: $("bb-settings-read-state"), deleteBurner: $("bb-delete-burner"), toast: $("bb-toast-stack") };
  var state = { page: 1, pageSize: Math.max(1, Number(cfg.pageSize) || 50), total: 0, loading: false, timer: 0, current: null, definitions: [], map: null, markers: null, mapItems: [], temperatureEnabled: false };
  ...
  function renderMap(items) {
    ...
    els.mapList.innerHTML = items.slice(0, 100).map(function (item) { return "<button class=\"bb-map-item\" data-map-ioid=\"" + esc(item.ioid) + "\"><span class=\"bb-map-item-dot\"></span><span class=\"bb-map-item-main\"><strong>" + esc(item.ioid) + "</strong><span>" + esc(item.location || item.name || "--") + "</span></span></button>"; }).join("");
    items.forEach(function (item) { var marker = window.L.marker([item.latitude, item.longitude], { title: item.ioid }); marker.bindPopup("<strong>" + esc(item.ioid) + "</strong><br>" + esc(item.stale ? "Mất kết nối" : modeLabel(item.mode))); marker._biomassIoid = item.ioid; state.markers.addLayer(marker); });
  }
  ...
  async function refresh() {
    ...
    try {
      ...
      var data = await api(url); state.total = data.list.total; state.definitions = data.settingDefinitions || [];
  ...
  els.body.onclick = function (event) { var button = event.target.closest("[data-settings]"); if (button) openSettings(button.dataset.settings); };
