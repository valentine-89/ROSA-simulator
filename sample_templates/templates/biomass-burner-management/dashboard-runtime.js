(function () {
  ...
  // Eight hardware levels: 0 is strongest, 56 is weakest (not percentages).
  var settingDefinitions = [
    { key: "1005", group: "Mồi lò", label: "Điện trở", minimum: 30, maximum: 180, unit: "s" },
    { key: "1007", group: "8 mức lửa", label: "Sơ cấp", list: true, unit: "" },
    { key: "1008", group: "8 mức lửa", label: "Thứ cấp", list: true, unit: "" }
  ];
  var cfg = {
  ...
  var els = {
    ...
    creditPurchased: $("bb-credit-purchased"), creditBurned: $("bb-credit-burned"), creditEdit: $("bb-credit-edit"),
  ...
  function semanticTelemetry(payload) {
    var source = payload || {};
    var names = ["mode", "burned_minutes", "program_version", "cfg_1005", "cfg_1007", "cfg_1008", "latitude", "longitude", "purchased_minutes_cache"];
    var normalized = Object.assign({}, source);
    names.forEach(function (name, index) { if (index >= 3 && source.c3 !== "compact-v2.8.0") return; if (normalized[name] == null && source["c" + (index + 1)] != null) normalized[name] = source["c" + (index + 1)]; });
    return normalized;
  }
  ...
  function validSettingValue(definition, value) { if (definition.list) return /^(?:[0-9]|[1-4][0-9]|5[0-6])(?:,(?:[0-9]|[1-4][0-9]|5[0-6])){7}$/.test(value); if (!Number.isInteger(value)) return false; if (Array.isArray(definition.choices)) return definition.choices.includes(value); return value >= definition.minimum && value <= definition.maximum; }
  function settingControl(definition, value) { if (definition.list) return '<input class="bb-input" name="value" type="text" maxlength="23" autocomplete="off" spellcheck="false" aria-label="' + esc(definition.label) + '" value="' + esc(value == null ? "" : value) + '">'; if (Array.isArray(definition.choices)) return "<select class=\"bb-input\" name=\"value\">" + definition.choices.map(function (choice) { return "<option value=\"" + choice + "\"" + (Number(value) === choice ? " selected" : "") + ">" + choice + "</option>"; }).join("") + "</select>"; return "<input class=\"bb-input\" name=\"value\" type=\"number\" step=\"1\" min=\"" + definition.minimum + "\" max=\"" + definition.maximum + "\" value=\"" + esc(value == null ? "" : value) + "\">"; }
  ...
  async function openSettings(ioid) { try { var rows = await runMacro(cfg.fleetViewPageId, "biomass-fleet-device", { burner_id: ioid }); if (!rows.length) throw new Error("Không tìm thấy lò"); if (!state.telemetry[ioid]) await loadBatchTelemetry(); var item = enriched(rows[0]); state.current = item; els.settingsTitle.textContent = "Cài đặt " + ioid; els.settingsSubtitle.textContent = deviceStateLabel(item) + (Number(item.mode) > 0 ? " · " + modeLabel(item.mode) : ""); els.coordinateSource.textContent = coordinateSourceLabel(item.coordinateSource); els.creditPurchased.textContent = number(item.purchasedMinutes); els.creditBurned.textContent = number(item.burnedMinutes); ["name", "location", "latitude", "longitude"].forEach(function (key) { els.deviceForm.elements[key].value = item[key] == null ? "" : item[key]; }); els.deviceForm.elements.api_key.value = ""; els.settingsGrid.innerHTML = settingsMarkup(item.payload); els.settingsState.textContent = item.credentialStatus === "accepted" ? "Đã đọc telemetry" : credentialLabel(item.credentialStatus); showModal(els.settingsModal, true); } catch (error) { notify(error.message, true); } }
  async function waitSetting(ioid, key, expected) { for (var attempt = 0; attempt < 8; attempt += 1) { await new Promise(function (resolve) { setTimeout(resolve, 1000); }); var latest = state.telemetry[ioid]; if (latest && String(latest.payload["cfg_" + key]) === String(expected)) return true; } return false; }
  async function auditSetting(ioid, key, value, resultState) { try { await runMacro(cfg.fleetAdminPageId, "biomass-setting-audit", { burner_id: ioid, setting_key: key, value: String(value), state: resultState }); } catch (_) {} }
