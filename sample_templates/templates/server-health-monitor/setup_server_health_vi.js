(function () {
  ...
  function normalizeConfig(source) {
    ...
    return {
      ...
      subtitle: safeText(cfg.subtitle, "Theo dõi CPU, RAM, network và dung lượng ổ đĩa chính cho nhiều server."),
  ...
  function shell() {
    document.body.innerHTML = ''
      + '<main class="shell">'
      + '<header class="head"><div><h1>Cài đặt Server Health Monitor</h1><div class="status" id="status">Đang chờ dữ liệu từ ứng dụng.</div></div><div class="badge" id="count-badge">0 server</div></header>'
      + '<section class="panel"><h2>Thông tin chung</h2><div class="grid" id="general-grid"></div></section>'
      + '<section class="panel"><h2>Ngưỡng cảnh báo</h2><div class="grid-3" id="threshold-grid"></div></section>'
      + '<section class="panel"><div class="toolbar"><h2>Danh sách server</h2><button type="button" data-action="add-server">Thêm server</button></div><div class="item-list" id="server-list"></div></section>'
      + '</main>';
    els = {
  ...
  function selectHtml(label, key, value, options) {
    ...
    if (selected && !hasSelected) {
      html += '<option value="' + esc(selected) + '" selected>' + esc(selected + " phút") + '</option>';
    }
    html += '</select></label>';
    return html;
  }
  ...
  function generalHtml(config) {
    return [
      inputHtml("Tên dashboard", "title", config.title, { full: true }),
      inputHtml("Mô tả ngắn", "subtitle", config.subtitle, { full: true }),
      inputHtml("Nhãn nhỏ phía trên", "eyebrow", config.eyebrow),
      inputHtml("AccountID / SyncID", "syncId", config.syncId),
      selectHtml("Khung thời gian mặc định", "historyMinutes", config.historyMinutes, HISTORY_RANGE_OPTIONS)
    ].join("");
  }
  ...
  function serverCard(server, index) {
    return ''
      ...
      + '<button class="secondary" type="button" data-action="clone-server" data-index="' + index + '">Nhân bản</button>'
      + '<button class="danger" type="button" data-action="remove-server" data-index="' + index + '">Xóa</button>'
      + '</div>'
      + '</div>'
      + '<div class="grid">'
      + serverInput("Tên server", "name", server.name)
      + serverInput("Device key / sessionId", "sessionId", server.sessionId, { full: true })
      + serverInput("Màu nhấn", "accent", server.accent, { type: "color" })
      + '</div>'
  ...
  function render() {
    ...
    els.serverList.innerHTML = state.servers.length
      ...
      : '<div class="empty">Chưa có server nào.</div>';
    els.countBadge.textContent = state.servers.length + " server";
    els.status.textContent = "Sẵn sàng cấu hình.";
  ...
  function onClick(event) {
    ...
    if (action === "remove-server") {
      if (state.servers.length <= 1) {
        els.status.textContent = "Cần giữ ít nhất một server.";
        return;
      }
  ...
  function collect() {
    ...
    if (!state.config.syncId) throw new Error("Hãy nhập SyncID.");
    if (!state.servers.length) throw new Error("Cần ít nhất một server.");
    state.servers.forEach(function (server, index) {
      if (!server.name) throw new Error("Server #" + (index + 1) + " chưa có tên.");
      if (!server.sessionId) throw new Error("Server #" + (index + 1) + " chưa có Device key.");
    });
