(function () {
  ...
  var language = "en";
  var phase = "single";
  var setupTitle = "Set up single-phase smart cabinet";
  ...
  function setupTitleText() {
    ...
    if (true) return isVi ? "Cài đặt tủ điện thông minh" : "Set up smart cabinet";
    ...
    if (false) return isVi ? "Cài đặt giám sát nhà máy" : "Set up factory monitoring";
  ...
  function listTitle() {
    ...
    if (true) return isVi ? "Danh sách tủ điện" : "Cabinet list";
  ...
  function addLabel() {
    ...
    if (true) return text.addCabinet;
  ...
  function setupShell() {
    ...
    var withList = false || true || false || false;
  ...
  function normalizeItems(preserveAdvanced) {
    ...
    if (true) state.items = state.items.map(makeSmartSession);
    if (false) state.items = state.items.map(makeIrrigationGroup);
    if (false) state.items = state.items.map(makeSequentialDevice);
  }
  ...
  function render() {
    ...
    els.countBadge.textContent = state.items.length + " " + (true ? text.cabinet : false ? "tuyến" : text.group);
    notifyResize();
  }

  function renderItem(item, index) {
    if (false) return renderBasicGroup(item, index);
    if (true) return renderSmartSession(item, index);
  ...
  function blankItem() {
    ...
    if (true) return { title: isVi ? "Tủ điện " + (state.items.length + 1) : "Cabinet " + (state.items.length + 1), sessionId: state.context.sessionId || "", description: "", switchCount: 2 };
  ...
  function cloneBasic(source) {
    ...
    if (true) return { title: safeText(source.title, text.cabinet) + " copy", sessionId: source.sessionId || state.context.sessionId || "", description: source.description || "", switchCount: source.switchCount || 2 };
