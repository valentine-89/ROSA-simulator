(function () {
      ...
      var settingsCurrentGroup = document.getElementById("settings-current-group");
      var settingsCurrentTitle = document.getElementById("settings-current-title");
      var settingsCurrentLowInput = document.getElementById("settings-current-low");
      var settingsCurrentHighInput = document.getElementById("settings-current-high");
      ...
      var settingsRunWindowGroup = document.getElementById("settings-run-window-group");
      var settingsRunWindowTitle = document.getElementById("settings-run-window-title");
      var settingsRunWindowStartInput = document.getElementById("settings-run-window-start");
      var settingsRunWindowEndInput = document.getElementById("settings-run-window-end");
      var settingsStopWindowGroup = document.getElementById("settings-stop-window-group");
      var settingsStopWindowTitle = document.getElementById("settings-stop-window-title");
      var settingsStopWindowStartInput = document.getElementById("settings-stop-window-start");
      var settingsStopWindowEndInput = document.getElementById("settings-stop-window-end");
      ...
      function applyStaticUiText() {
        ...
        setNodeText(settingsCurrentTitle, getUi("settingsModal.currentTitle", "Current threshold"));
        setNodeText(settingsPhoneTitle, getUi("settingsModal.phoneTitle", "Alarm phone number"));
        setNodeText(settingsRunWindowTitle, getUi("settingsModal.runWindowTitle", "Run window"));
        setNodeText(settingsStopWindowTitle, getUi("settingsModal.stopWindowTitle", "Stop window"));
        ...
        setNodeText(thresholdForm ? thresholdForm.querySelector('label[for="settings-current-low"]') : null, getUi("settingsModal.lowLabel", "Low threshold"));
        setNodeText(thresholdForm ? thresholdForm.querySelector('label[for="settings-current-high"]') : null, getUi("settingsModal.highLabel", "High threshold"));
        setNodeText(thresholdForm ? thresholdForm.querySelector('label[for="settings-alarm-phone"]') : null, getUi("settingsModal.phoneLabel", "Phone number"));
        setNodeText(thresholdForm ? thresholdForm.querySelector('label[for="settings-run-window-start"]') : null, getUi("settingsModal.startLabel", "Start"));
        setNodeText(thresholdForm ? thresholdForm.querySelector('label[for="settings-run-window-end"]') : null, getUi("settingsModal.endLabel", "End"));
        setNodeText(thresholdForm ? thresholdForm.querySelector('label[for="settings-stop-window-start"]') : null, getUi("settingsModal.startLabel", "Start"));
        setNodeText(thresholdForm ? thresholdForm.querySelector('label[for="settings-stop-window-end"]') : null, getUi("settingsModal.endLabel", "End"));
      ...
      function responseMatchesCommandEcho(text, command) {
        ...
        return normalizedText.indexOf(normalizedCommand) >= 0;
      }
      ...
      /* ── Control overlay: dim + spinner + 3 s cooldown ── */
      var CONTROL_COOLDOWN_MS = 3000;
      ...
      function getContactorDefinitions(state) {
        var definitions = [
          { key: "contactor1", label: getUi("contactor.labels.contactor1", "Contactor 1"), code: "O1", onKey: "contactor1On", offKey: "contactor1Off" },
          { key: "contactor2", label: getUi("contactor.labels.contactor2", "Contactor 2"), code: "O2", onKey: "contactor2On", offKey: "contactor2Off" },
          { key: "contactor3", label: getUi("contactor.labels.contactor3", "Contactor 3"), code: "O3", onKey: "contactor3On", offKey: "contactor3Off" }
        ];
      ...
      function createMeterStates(state) {
        ...
        state.contactorDefinitions.forEach(function (definition) {
          ...
          result[definition.key] = {
            key: definition.key,
            label: safeText(configuredFields.title, definition.label),
      ...
      function renderPond(state) {
        ...
        state.hasSettings = !!state.commandApiUrl && (hasField(state, 'currentThreshold') || hasField(state, 'alarmPhone') || hasField(state, 'runWindow') || hasField(state, 'stopWindow') || hasField(state, 'updateEnabled'));
      ...
      function getMeterHistoryFields(state) {
        var fields = [];
        Object.keys(state && state.meterStates || {}).forEach(function (meterKey) {
          var meter = state.meterStates[meterKey];
      ...
      function loadMeterHistory(state, force) {
        ...
        var fields = getMeterHistoryFields(state);
        if (!fields.length) return Promise.resolve();
        if (!force && state.meterHistoryWindowMs === historyMs && state.meterHistoryLoaded) {
      ...
      function updateTelemetryField(state, field, value, timestamp) {
        ...
        if (!matched) {
          ...
          if (currentThresholdMatch.matched) {
            state.thresholds.current = parseThresholdValue(currentThresholdMatch.value);
            if (meterModalState.state === state) queueMeterModalRefresh();
            matched = true;
          }
        }
        ...
        if (!matched) {
          ...
          if (runWindowMatch.matched) {
            state.runWindow = parseWindowValue(runWindowMatch.value);
            matched = true;
          }
        }
        if (!matched) {
          var stopWindowMatch = extractConfiguredTelemetryValue(key, state.fields.stopWindow, value);
          if (stopWindowMatch.matched) {
            state.stopWindow = parseWindowValue(stopWindowMatch.value);
            matched = true;
          }
        }
        if (!matched) {
      ...
      function connectPond(state) {
        ...
        var streamFields = [
          ...
          .concat(meterFields.reduce(function (result, value) {
            return result.concat(expandConfiguredStreamFields(value));
          }, []))
          .filter(Boolean)
        ...
        ensurePondBootstrap(state).finally(function () {
          ...
          state.eventSource = new EventSource(buildStreamUrl(state.sessionId, streamFields, getHistoryWindowMs()));
      ...
      function openSettingsModal(state) {
        ...
        var hasStopWindow = hasField(state, 'stopWindow');
        var hasUpdateEnabled = hasField(state, 'updateEnabled');
        setSettingsGroupVisibility(settingsCurrentGroup, hasCurrent);
        setSettingsGroupVisibility(settingsPhoneGroup, hasPhone);
        setSettingsGroupVisibility(settingsRunWindowGroup, hasRunWindow);
        setSettingsGroupVisibility(settingsStopWindowGroup, hasStopWindow);
        setSettingsGroupVisibility(settingsUpdateGroup, hasUpdateEnabled);

        var currentThreshold = state.thresholds.current || { low: null, high: null };
        var runWindow = state.runWindow || { start: "", end: "", raw: "" };
        var stopWindow = state.stopWindow || { start: "", end: "", raw: "" };
        settingsCurrentLowInput.value = hasCurrent && currentThreshold.low !== null ? String(currentThreshold.low) : '';
        settingsCurrentHighInput.value = hasCurrent && currentThreshold.high !== null ? String(currentThreshold.high) : '';
        settingsAlarmPhoneInput.value = hasPhone ? String(state.alarmPhone || '') : '';
        settingsRunWindowStartInput.value = hasRunWindow ? String(runWindow.start || '') : '';
        settingsRunWindowEndInput.value = hasRunWindow ? String(runWindow.end || '') : '';
        settingsStopWindowStartInput.value = hasStopWindow ? String(stopWindow.start || '') : '';
        settingsStopWindowEndInput.value = hasStopWindow ? String(stopWindow.end || '') : '';

        settingsCurrentLowInput.placeholder = hasCurrent && currentThreshold.low === null ? getUi('thresholds.placeholders.currentLow', '0.5') : '';
        settingsCurrentHighInput.placeholder = hasCurrent && currentThreshold.high === null ? getUi('thresholds.placeholders.currentHigh', '3.5') : '';
        settingsAlarmPhoneInput.placeholder = hasPhone && !state.alarmPhone ? getUi('thresholds.placeholders.alarmPhone', '+84901234567') : '';
        settingsRunWindowStartInput.placeholder = hasRunWindow && !runWindow.start ? getUi('thresholds.placeholders.runWindowStart', '0700') : '';
        settingsRunWindowEndInput.placeholder = hasRunWindow && !runWindow.end ? getUi('thresholds.placeholders.runWindowEnd', '2359') : '';
        settingsStopWindowStartInput.placeholder = hasStopWindow && !stopWindow.start ? getUi('thresholds.placeholders.stopWindowStart', '0000') : '';
        settingsStopWindowEndInput.placeholder = hasStopWindow && !stopWindow.end ? getUi('thresholds.placeholders.stopWindowEnd', '0000') : '';
      ...
      function refreshMeterModal() {
        ...
        meterModalChartState.thresholds = state.thresholds || {};
      ...
      var pondStates = (Array.isArray(config.sessions) ? config.sessions : []).map(function (sessionConfig, index) {
        ...
        return {
          ...
          thresholds: { current: { low: null, high: null } },
          alarmPhone: '',
          runWindow: { start: "", end: "", raw: "" },
          stopWindow: { start: "", end: "", raw: "" },
          updateEnabled: null,
          meterStates: {},
      ...
      thresholdForm.addEventListener('submit', function (event) {
        ...
        var commands = [];
        var nextCurrent = state.thresholds.current;
        var nextAlarmPhone = state.alarmPhone;
        var nextRunWindow = state.runWindow;
        var nextStopWindow = state.stopWindow;

        if (hasField(state, 'currentThreshold')) {
          var currentLow = numericValue(settingsCurrentLowInput.value);
          var currentHigh = numericValue(settingsCurrentHighInput.value);
          ...
          var currentRaw = currentLow + ',' + currentHigh;
          nextCurrent = { low: currentLow, high: currentHigh, raw: currentRaw };
          if ((state.thresholds.current && state.thresholds.current.raw) !== currentRaw) {
            commands.push({ command: state.fields.currentThreshold + '=' + currentRaw, apply: function () { state.thresholds.current = nextCurrent; } });
          }
        }
        ...
        if (hasField(state, 'runWindow')) {
          var runStartRaw = String(settingsRunWindowStartInput.value || '').trim();
          var runEndRaw = String(settingsRunWindowEndInput.value || '').trim();
          ...
          var runWindowRaw = runWindowStart && runWindowEnd ? (runWindowStart + ',' + runWindowEnd) : '';
          nextRunWindow = { start: runWindowStart, end: runWindowEnd, raw: runWindowRaw };
          if ((state.runWindow && state.runWindow.raw) !== runWindowRaw) {
            commands.push({ command: state.fields.runWindow + '=' + runWindowRaw, apply: function () { state.runWindow = nextRunWindow; } });
          }
        }

        if (hasField(state, 'stopWindow')) {
          var stopStartRaw = String(settingsStopWindowStartInput.value || '').trim();
          var stopEndRaw = String(settingsStopWindowEndInput.value || '').trim();
          var stopWindowStart = normalizeCompactTimeValue(stopStartRaw);
          var stopWindowEnd = normalizeCompactTimeValue(stopEndRaw);
          if ((stopStartRaw && !stopWindowStart) || (stopEndRaw && !stopWindowEnd)) {
            showToast(getUi('thresholds.timeWindowInvalid', 'Time must use 4 digits in HHMM format.'), 'error');
            return;
          }
          if ((stopWindowStart && !stopWindowEnd) || (!stopWindowStart && stopWindowEnd)) {
            showToast(getUi('thresholds.timeWindowBothRequired', 'Both start and end times are required.'), 'error');
            return;
          }
          var stopWindowRaw = stopWindowStart && stopWindowEnd ? (stopWindowStart + ',' + stopWindowEnd) : '';
          nextStopWindow = { start: stopWindowStart, end: stopWindowEnd, raw: stopWindowRaw };
          if ((state.stopWindow && state.stopWindow.raw) !== stopWindowRaw) {
            commands.push({ command: state.fields.stopWindow + '=' + stopWindowRaw, apply: function () { state.stopWindow = nextStopWindow; } });
          }
        }
        ...
        var previousStopWindow = state.stopWindow;
        ...
        withControlCooldown(settingsModalCard, function () {
          ...
          return chain.then(function () {
            ...
            state.stopWindow = previousStopWindow;
            if (meterModalState.state === state) queueMeterModalRefresh();
