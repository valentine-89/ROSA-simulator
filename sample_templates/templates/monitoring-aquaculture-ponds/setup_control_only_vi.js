(function () {
  ...
  var T = getSetupText("control-only", isVi);
  var state = {
    baseConfig: {},
    context: {},
    items: []
  ...
  function basicTitle(item, index) {
    if (true) return safeText(item.labelPrefix, T.device + " " + (index + 1));
    return safeText(item.title, (false ? "Controller " : T.controller + " ") + (index + 1));
  }
