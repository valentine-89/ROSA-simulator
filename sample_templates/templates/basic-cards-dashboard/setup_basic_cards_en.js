(function () {
  ...
  function setupTitleText() {
    ...
    if (true) return isVi ? "Cài đặt trang thẻ cơ bản" : "Set up basic cards";
  ...
  function listTitle() {
    if (true) return isVi ? "Danh sách nhóm thiết bị" : "Device groups";
  ...
  function setupShell() {
    ...
    var withList = true || false || false || false;
  ...
  function normalizeItems(preserveAdvanced) {
    if (true) state.items = state.items.map(makeBasicGroup);
  ...
  function renderItem(item, index) {
    if (true) return renderBasicGroup(item, index);
  ...
  function blankItem() {
    if (true) return { title: isVi ? "Thiết bị " + (state.items.length + 1) : "Device " + (state.items.length + 1), sessionId: state.context.sessionId || "", widgetCount: 3 };
  ...
  function handleWidgetAction(action, groupIndex, widgetIndex) {
    if (false || groupIndex < 0 || !state.items[groupIndex]) return false;
